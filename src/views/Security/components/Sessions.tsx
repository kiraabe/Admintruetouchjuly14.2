...
const Sessions = () => {
  ...
  const handleRevokeSession = async (session: Session) => {
    ...
    try {
      // TODO: replace with real API call, e.g. DELETE /api/sessions/:id
      await new Promise((resolve) => setTimeout(resolve, 800))

      // Audit log the forced logout of the remote session
      // (The remote session will be expired server-side; we record the admin action)
      await SessionService.expireSession(
        ...
        LogoutType.FORCED,
        {
          ...
          revokedLocation:  session.location,
  ...
  const handleRevokeAllOthers = async () => {
    if (
      !window.confirm('This will sign you out from all other devices. Continue?')
    ) return

    setLoading('all')
    try {
      // TODO: replace with real API call, e.g. DELETE /api/sessions/others
      await new Promise((resolve) => setTimeout(resolve, 1000))

      const revoked = sessions.filter((s) => !s.isCurrent)
      ...
      await Promise.allSettled(
        revoked.map((s) =>
          SessionService.expireSession(
            ...
            LogoutType.FORCED,
            {
              revokedSessionId: s.id,
              revokedDevice:    s.device,
              revokedBrowser:   s.browser,
              revokedIp:        s.ip,
              revokedLocation:  s.location,
              action:           'admin_revoke_all_other_sessions',
            }
          )
        )
      )
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  <h6 className="font-semibold">{session.device}</h6>
                  {session.isCurrent && (
